...
export async function PATCH(request: NextRequest) { const me=await currentUser(request); if(me?.role!=="admin") return NextResponse.json({error:"Forbidden"},{status:403}); const body=await request.json(); const status=body.status==="approved"?"approved":"inactive"; await getDb().update(users).set({status}).where(eq(users.id,Number(body.id))); return NextResponse.json({ok:true}); }
