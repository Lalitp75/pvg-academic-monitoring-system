...
export default function Home() {
  ...
  async function setTemporaryPassword(id:number,name:string){const temporaryPassword=window.prompt(`Enter a temporary password for ${name}. It must contain at least 8 characters.`);if(temporaryPassword===null)return;if(temporaryPassword.length<8){window.alert("Temporary password must contain at least 8 characters.");return;}const r=await fetch("/api/admin/users",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,action:"setTemporaryPassword",temporaryPassword})});const data=await r.json();if(r.ok)window.alert(`Temporary password updated for ${name}.\n\nFaculty can now sign in with this password.\nNew Recovery Code: ${data.recoveryCode}\n\nShare both securely. Existing attendance records are unchanged.`);else window.alert(data.error||"Unable to reset password.");}
  ...
  return (
    <main className="app-shell">
      ...
      {user.role==="admin"&&<section className="panel approvals-panel"><div className="panel-heading approvals-heading"><div><p className="section-kicker">Access control</p><h3>Faculty Account Approvals</h3></div><div className="approvals-controls"><span>{staffUsers.filter(s=>s.status==="pending").length} pending</span><Button type="button" size="sm" variant="outline" onClick={()=>setApprovalsOpen(open=>!open)} aria-expanded={approvalsOpen}>{approvalsOpen?<><ChevronUp/>Minimize</>:<><ChevronDown/>View Approvals</>}</Button></div></div>
        {approvalsOpen&&<div className="approval-list">{staffUsers.length?staffUsers.map(s=><div className="approval-row" key={s.id}><div><strong>{s.name}</strong><small>{s.email} · {s.department}</small></div><span className={`account-status ${s.status}`}>{s.status}</span><div><Button size="sm" onClick={()=>updateStaff(s.id,"approved")}>Approve</Button><Button size="sm" variant="outline" onClick={()=>generateStaffRecoveryCode(s.id,s.name)}>Recovery Code</Button><Button size="sm" variant="outline" onClick={()=>setTemporaryPassword(s.id,s.name)}>Set Temporary Password</Button><Button size="sm" variant="outline" onClick={()=>updateStaff(s.id,"inactive")}>Deactivate</Button></div></div>):<p className="empty">No faculty registrations yet.</p>}</div>}
      </section>}
