...
export default function Home() {
  ...
  async function setTemporaryPassword(id:number,name:string){const password=window.prompt(`Enter a new password for ${name} (minimum 8 characters):`);if(password===null)return;if(password.length<8){window.alert("Password must contain at least 8 characters.");return;}const confirmPassword=window.prompt("Enter the same password again to confirm:");if(confirmPassword===null)return;if(password!==confirmPassword){window.alert("Passwords do not match. Nothing was changed.");return;}const r=await fetch("/api/admin/users",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,action:"setTemporaryPassword",password})});const data=await r.json();if(r.ok)window.alert(`Password reset completed for ${name}.\n\nEmail: ${data.email}\nRecovery Code: ${data.recoveryCode}\n\nShare the password and Recovery Code securely. Existing attendance records are unchanged.`);else window.alert(data.error||"Unable to reset password.");}
