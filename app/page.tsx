...
export default function Home() {
  ...
  async function setTemporaryPassword(id:number,name:string){if(!window.confirm(`Generate a new temporary password for ${name}?`))return;const r=await fetch("/api/admin/users",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,action:"setTemporaryPassword"})});const data=await r.json();if(r.ok)window.alert(`Temporary login details for ${name}:\n\nEmail: ${data.email}\nTemporary Password: ${data.temporaryPassword}\nRecovery Code: ${data.recoveryCode}\n\nCopy and share them securely. Existing attendance records are unchanged.`);else window.alert(data.error||"Unable to reset password.");}
